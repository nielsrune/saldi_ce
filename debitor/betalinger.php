...
#cho "select ordre_id, bilag_id from betalinger<br>";
	...
	$qtxt="select openpost.id as id,openpost.beskrivelse as egen_ref,openpost.amount as amount,openpost.valuta as valuta,openpost.faktnr as faktnr,openpost.transdate as transdate,openpost.bilag_id as bilag_id,openpost.forfaldsdate as forfaldsdate,openpost.betal_id as betal_id,adresser.erh as erh, openpost.refnr as refnr, openpost.kladde_id as kladde_id, adresser.bank_reg as modt_reg, adresser.bank_konto as modt_konto, adresser.firmanavn as modt_navn,adresser.bank_fi as modt_fi,adresser.betalingsbet as betalingsbet,adresser.betalingsdage as betalingsdage from openpost, adresser where openpost.udlignet != '1' and openpost.amount < 0 and openpost.konto_id = adresser.id and adresser.art = 'D'";
#cho "qtxt $qtxt<br>";
...
#cho __LINE__."<br>";		
			...
			$linje.='","';
			$tmp=usdecimal($r['belob'])*100; #4 Transbeløb
			$sum+=$tmp;
			...
			$linje.='","';
			$tmp=sprintf("%-20s", substr(iconv("UTF-8", "Windows-1252//IGNORE", $r['egen_ref']),0,20)); #12cPosteringstekst
			...
			$linje.='","';
/*			
			$r2=db_fetch_array(db_select("select firmanavn,addr1,addr2,postnr,bynavn from ordrer where id=$r['ordre_id']",__FILE__ . " linje " . __LINE__));
			$tmp=sprintf("%-20s", substr(iconv("UTF-8", "Windows-1252//IGNORE", $r2['firmanavn']),0,32)); #Navn
			while (strlen($tmp)<32)$tmp.=" ";
			$linje.=$tmp;
			$tmp=sprintf("%-20s", substr(iconv("UTF-8", "Windows-1252//IGNORE", $r2['addr1']),0,32)); #Adresse-1
			while (strlen($tmp)<32)$tmp.=" ";
			$linje.=$tmp;
			$tmp=sprintf("%-20s", substr(iconv("UTF-8", "Windows-1252//IGNORE", $r2['addr2']),0,32)); #Adresse-2
			while (strlen($tmp)<32)$tmp.=" ";
			$linje.=$tmp;
			$tmp=sprintf("%-20s", substr(iconv("UTF-8", "Windows-1252//IGNORE", $r2['addr2']),0,4)); #Postnr
			while (strlen($tmp)<4)$tmp.=" ";
			$linje.=$tmp;
			$tmp=sprintf("%-20s", substr(iconv("UTF-8", "Windows-1252//IGNORE", $r2['bynavn']),0,32)); #Bynavn
			while (strlen($tmp)<32)$tmp.=" ";
			$linje.=$tmp;
*/
			$tmp='';
