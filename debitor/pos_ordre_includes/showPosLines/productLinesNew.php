...
// 20230216 PHR	Moved all sum & nettosum calculations to ordrelinjerDataII.php    
			...
			if ($rabatantal[$x]) {
				...
				if ($grupperabat) {
					if ($posnr[$x] && $show) {
						...
						file_put_contents("../temp/$db/customerDisplay.txt",$rabatantal[$x]."\trabat\t"
						.$grupperabat*$rabatantal[$x]."\n",FILE_APPEND);
						$displayLine[$dx]  = $linje_id[$x];
						$displayTxt[$dx]   = 'Rabat';
...
#cho __line__." S $sum<br>";
				...
				if ($posnr[$x] && $show) {
					...
					file_put_contents("../temp/$db/customerDisplay.txt",$antal[$x]."\trabat\t"
					.$antal[$x]*$m_rabat[$x]."\n",FILE_APPEND);
					$displayLine[$dx]  = $linje_id[$x];
					$displayTxt[$dx]   = 'Rabat';
					$displayQty[$dx]   = $antal[$x]; 
					$displayPrice[$dx] = $antal[$x]*$m_rabat[$x];
					$dx++;
#					kundedisplay($tmp,$antal[$x]*$m_rabat[$x],0,$antal[$x]);
				}
