...
//
// --- debitor/pos_ordre.php --- lap 4.0.6 --- 2022.07.26 ---
// LICENSE
// 
...
$title="POS_ordre";
$css="../css/pos.css";
$addRemove=$afd=$afslut=$afd_lager=$afd_navn=NULL;
$barcodeNew=$beskrivelse_ny=$betaling=$betaling2=$bordnavn=NULL;
$country=NULL;
...
$rabat_ny=$ref=NULL;
$saldi_bet=$saldi_pfs=$skift_bruger=$status=$svar=$svnr=NULL;	
$valuta='DKK';$valutakurs='100';$vare_id=$vis_kassenr=NULL;

$antal_ny = $bordnr = 0;
$bord=$koekkenprinter=$lagernr=$lagernavn=$saldi_bordnr=array();
...
if ($totalrabat && $id) {
	...
	if ($totalrabat) {
		...
		while ($r=db_fetch_array($q)) {
			if ($r['rabat']) {
#				echo "$totalrabat > $r[rabat]<br>";
				$newDiscount = $totalrabat+$r['rabat']-($r['rabat']*$totalrabat/100);
#				echo __line__."$totalrabat > $r[rabat] > $newDiscount<br>";
...
echo __line__." antalNy $antal_ny<br>";
...
echo __line__." antalNy $antal_ny - $_POST[antal] | $_POST[antal_ny]<br>";
		if (!$antal_ny && $antal_ny!='0') $antal_ny=$_POST['antal_ny'];
...
#		$bon=NULL;
	...
	if ($fokus=="antal_ny" && $antal_ny!='0' && !$pris_ny && substr($antal_ny,-1) != 't') $antal_ny.="p";
...
if (!$id || $id == 0) {
	...
	if (count($bord) && is_numeric($bordnr)) { #20210930
...
function opret_posordre($konto_id,$kasse){
	...
	$r = db_fetch_array(db_select("select box4 from grupper where art = 'POS' and kodenr='2'",__FILE__ . " linje " . __LINE__)); 
		...
		$tmp=explode(chr(9),$r['box4']);
		$terminal_ip=trim($tmp[$x]);
		if (isset($_COOKIE['salditerm'])) $terminal_ip=$_COOKIE['salditerm'];
		if ($terminal_ip=='box' || $terminal_ip=='saldibox') {
		$filnavn="http://saldi.dk/kasse/".$_SERVER['REMOTE_ADDR'].".ip";
		if ($fp=fopen($filnavn,'r')) {
			$terminal_ip=trim(fgets($fp));
...
#		print "<td><input class='inputbox' type='text' style='text-align:right;font-size:$ifs;width:25px' name='pfs' value='$pfs'></td></tr>";
		...
		for($x=0;$x<count($afd);$x++) {
			$kasse=$x+1;
			if (!count($afd_nr) && $kasse==$nuv_kasse) print "<option value=\"$kasse\">$kasse: $afd_navn[$x]</option>\n";
