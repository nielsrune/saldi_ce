...
//
// ------------- api/varesync.php ---------- lap 3.7.0----2017.08.08-------
// LICENS
//
...
#cho __line__." ".$qtxt."<br>";
		$r=db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
		$vare_id[$y]=$r['id'];
#cho __line__." Vare_id $vare_id[$y] && $shop_id[$y]<br>";
...
#cho __line__." Valg $valg Vare_id $vare_id[$y] && $shop_id[$y] S $salgspris[$y] G $gruppe[$y]<br>";
		if ($vare_id[$y]) {
			...
			$r=db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
			if ($r['id'] && $shop_id[$y])	$qtxt="update shop_varer set shop_id='$shop_id[$y]' where id='$r[id]'";
			elseif ($shop_id[$y]) $qtxt="insert into shop_varer (saldi_id,shop_id) values ('$vare_id[$y]','$shop_id[$y]')";
#cho __line__." $qtxt<br>";
			db_modify($qtxt,__FILE__ . " linje " . __LINE__);
...
#cho __line__." $variant_type[$y]==$var_type[$x]<br>";
				$s_var_id[$y]=$var_id[$x];
			}
		}
...
#cho __line__." $parent_id[$y] && $s_var_id[$y] && $variant[$y]<br>";		
			for ($x=0;$x<count($vt_id);$x++) {
				if ($s_var_id[$y]==$vt_var_id[$x] && $vt_var[$x]==$variant[$y]) {
#cho __line__." $s_var_id[$y]==$vt_var_id[$x] && $vt_var[$x]==$variant[$y]<br>";
					$s_variant[$y]=$vt_id[$x];
...
#cho __line__." $qtxt<br>";				
				...
				$r=db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
				$s_variant_id[$y]=$r['id'];
				$qtxt="select id from shop_varer where shop_variant='$variant_id[$y]' or saldi_variant='$s_variant_id[$y]'";
#cho __line__." ".$qtxt."<br>";
				$r=db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
				if ($r['id']) {
