...
//
// --- index/login.php --- lap 4.0.3 --- 2021-11-05 ---
// LICENSE
//
...
$qtxt = "select var_value from settings where var_name = 'language_id' and user_id = '0'";
$r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__)); 
if($r){ #20211018
...
if ($db) {
	...
	$qtxt = "insert into online (session_id, brugernavn, db, dbuser, logtime) values ";
	$qtxt.= "('$s_id', '".db_escape_string($brugernavn)."', '$db', '$dbuser', '$unixtime')";
	db_modify($qtxt,__FILE__ . " linje " . __LINE__);
}
else db_modify("delete from online where db=''",__FILE__ . " linje " . __LINE__); 

## Versions kontrol / opdatering af database.
#echo __line__." $regnskab && $regnskab!=$sqdb<br>";
if ( $db && $db!=$sqdb ) {
...
#include("../includes/online.php"); #20211005 This is causing error..why is this here?
$bruger_id=NULL;
...
if ($bruger_id) {
	$qtxt = "select var_value from settings where var_name = 'languageId' and user_id = '0'";
	$r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
	($r['var_value'])?$languageId = $r['var_value']:$languageId = 1; #20211205
...
#		$r=db_fetch_array(db_select("select box4 from grupper where art='API'",__FILE__ . " linje " . __LINE__));
	...
	if (!$sag_rettigheder&&$rettigheder) {    
		...
		##########################
		if ($db=='pos_13') {
			alert("Vi har forsøgt at få kontakt til jer, både pr mail og telefon, men kommer ikke igennem.\\nRing venligst 4690 2208.\\nMvh Saldi.");
		}
