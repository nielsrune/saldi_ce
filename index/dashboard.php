...
if (!check_permissions(array(3,4))) {
	...
	if ($state) {
		...
		#print "<button style='padding: 1em; cursor: pointer' onclick='window.location.href=\"../debitor/pos_ordre.php\"'>Åben kassesystem</button>";
		
		#print "<button id='toggleButton' style='padding: 1em; cursor: pointer;' onclick='handleButtonClick()'>Åben kassesystem</button>";
	} else {
...
if ($ordercount === "on") {
	...
	$q=db_select("SELECT count(*), COALESCE(sum(\"sum\"), 0) FROM ordrer WHERE status < 3 AND art = 'DO' AND ordredate > '$thirtyDaysAgo'",__FILE__ . " linje " . __LINE__);
...
<div style='display: none' id='settingpopup'>
  <div style='top: 0; position: fixed; height: 100vh; width: 100vw; background-color: #00000030'>
  </div>
