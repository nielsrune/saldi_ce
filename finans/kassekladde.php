...
//
// --- finans/kassekladde.php ----------------- ver 4.0.8 -- 2023-03-22 --
// LICENSE
//
...
$afd=$amount=$ansat=$ansat_id=$belob=$beskrivelse=$betal_id=$bilag=$dato=$d_type=$debet=$faktura=$forfaldsdate=$forfaldsdato=array();
$id=$k_type=$kontonr=$kredit=$lobenr=$momsfri=$projekt=$valuta=array();
...
$intern_bilag=NULL;
$k_type_ligslut=$k_type_ligstart=$kontrolkonto=$kontrolsaldo=NULL;
$kredit_ligslut=$kredit_ligstart=$kladde_id=$kladdenote=$regnstart=NULL;
...
if($_GET) {
	...
	if (!isset($forfaldsdato[$x])) $forfaldsdato[$x] = NULL;
	if (!isset($betal_id[$x])) $betal_id[$x] = '';
	if (!is_numeric($kladde_id))  $kladde_id  = 0;
...
#		while (strpos($beskrivelse[$x],"''")) $beskrivelse[$x]=str_replace("''","'",$beskrivelse[$x]); #20130731
		...
		if ( (int)$bilag[$x] ) { #20230302 - you can't subtract 1 from a string
			if ($submit!='lookup' && (!$bilag[$x] || $bilag[$x]==$bilag[$x-1] || $bilag[$x]-1==$bilag[$x-1]) && (!$dato[$x] || $dato[$x]==$dato[$x-1]) && (!$beskrivelse[$x] || $beskrivelse[$x]==$beskrivelse[$x-1]) && !$d_type[$x] && !$k_type[$x] && !$debet[$x] && !$kredit[$x] && !$faktura[$x] && !$belob[$x]) { 
				$bilag[$x]='-';#20170418
			} elseif ($submit!='lookup' && (!$bilag[$x] || $bilag[$x]==$bilag[$x-1]) && (!$dato[$x] || $dato[$x]==$dato[$x-1]) && (!$beskrivelse[$x] || $beskrivelse[$x]==$beskrivelse[$x-1]) && !$d_type[$x] && !$k_type[$x] && !$debet[$x] && !$kredit[$x] && !$faktura[$x] && !$belob[$x]) {
				$bilag[$x]='-';#20230302
			}
		}
...
#cho __line__." $submit $debet[$x] $fokus $x<br>";
	...
	if ($kladde_id) {
		...
		if (!$row['bogfort'] && $tidspkt==$row['tidspkt']) { #Refreshtjek"
			...
			if ($submit == 'lookup') $opslag_id=substr($fokus,4,strlen($fokus)-4);
			elseif ($kladde_id) {
				$row =db_fetch_array(db_select("select bogfort from kladdeliste where id = $kladde_id",__FILE__ . " linje " . __LINE__));
				if ($row['bogfort']=='-') {
...
#	} elseif ($bogfort!="S") {
		...
		print "<td width=5px></td><td width=\"80px\" align=\"center\"><span title=\"".findtekst(1561, $sprog_id)."\"><input type='submit' class='button gray small' style=\"width:120px;float:left\" accesskey=\"o\" value=\"".findtekst(1091, $sprog_id)."\" name=\"updateNote\" onclick=\"javascript:docChange = false;\"></span></td>";
	}
...
#		$beskrivelse[$x]=htmlentities($row['beskrivelse'],ENT_QUOTES,$charset);
		...
		if ((($k_type[$x]=='D')||($k_type[$x]=='K'))&&($kredit[$x])&&(!$fejl)) {
			...
			if ($row2 = db_fetch_array($query2)) $kredittext[$x]=trim($row2['firmanavn']);
			else $kredittext[$x] = '';
			$tmpffdato=forfaldsdag($transdate[$x],$row2['betalingsbet'],$row2['betalingsdage']);
		}
...
#		if (($k_type[$y]!="D")&&($k_type[$y]!="K")) $k_type[$y]="F"; #phr 20070801
	...
	if(!isset($bilag[0])   ) $bilag[0]    = NULL; 
	if(!isset($bilag[$x+1])) $bilag[$x+1] = NULL;
	if(!isset($dato[0])    ) $dato[0]     = NULL; 
	if(!isset($dato[$x+1]) ) $dato[$x+1]=   NULL;
	for ($y=1;$y<=$x;$y++) {
		if (!isset($bilag[$y])     ) $bilag[$y]      = NULL; 
...
#		} else $dk_amount=$amount[$y];
		...
		if ($kontrolkonto) {
			...
			if ($d_type[$y]=='F' && $debet[$y]==$kontrolkonto) $kontrolsaldo+=$dkkamount[$y]/$tmp; # 20230302
			if ($k_type[$y]=='F' && $kredit[$y]==$kontrolkonto) $kontrolsaldo+=-$dkkamount[$y]/$tmp; # 20230302
			if ( $debet[$y]==='' && $kredit[$y]==='' ) $kontrolsaldo="";
		}
...
#cho __line__." $submit $debet[$x] $fokus $x<br>";
		if (($d_type=="F")&&(strlen($debet)>1)&&(!is_numeric($debet))) {
			$tmp=$debet."%";
			$i=0;
			$qtxt = "select kontonr from kontoplan where ";
			$qtxt.= "beskrivelse like '$tmp' and regnskabsaar='$regnaar' and lukket != 'on' order by beskrivelse";
#cho "$qtxt<br>";
			$query = db_select($qtxt,__FILE__ . " linje " . __LINE__);
			while ($row = db_fetch_array($query)) {
				$i++;
			...
			elseif ($i>1) {
				...
				if ($row = db_fetch_array($query)) $debet=$row['kontonr'];
					...
					$alerttekst=addslashes($txt1."'".$debet."' ".$txt2." ".$bilag.$txt3); # 20230306 added ' and spaces
					alert($alerttekst);
					$fejl=1;
				}
			}
		...
		if ((!$fejl)&&($d_type=="F")&&($debet>0)) {
			...
			} elseif (in_array($debet,$lukket)) $alerttekst=addslashes($alerttxt4." ".$debet." ".$alerttxt5." ".$bilag.$alerttxt3); # 20230306 added spaces
			if ($alerttekst) {
				alert("$alerttekst");
				$fejl=1;
			}
		...
		if ((!$fejl)&&($k_type=="F")&&($kredit>0)) {
			...
			} elseif (in_array($kredit,$lukket)) $alerttekst=addslashes($alert1." ".$kredit." ".$alerttxt4." ".$bilag.$alert4);
			if ($alerttekst) {
				alert($alerttekst);
				$fejl=1;
			}
...
#			else $transdate=date("Y-m-d"); # <- 2009.05.14
			...
			if ($amount < 0) {# Hvis beloebet er negativt, byttes om paa debet og kredit.
 			...
 			if ($r['id'] && ($r['bilag'] || $r['bilag']=='0')) {
				...
				} else $qtxt = "update kassekladde set forfaldsdate=NULL, betal_id='' where id = '$r[id]'";
				db_modify($qtxt,__FILE__ . " linje " . __LINE__);
			} elseif (($r['transdate'] || $transdate) 
...
#cho "$kladde_id,$bilagsnr,$ny_dato,$vend_fortegn<br>";
		...
		print "<input type=hidden name=kladde_id value=$kladde_id>";
		print "<input class='button green medium' type='submit' accesskey=\"k\" value=\"".findtekst(1598, $sprog_id)."\" name=\"copy2new\" onclick=\"javascript:docChange = false;\">&nbsp;<input class='button rosy medium' type=button value=Fortryd onClick=\"location.href='../finans/kladdeliste.php'\"><br></span>\n";
		print "</form>";
		exit;
	}
exit;
