...
//
// --- finans/kassekladde.php --- ver 4.0.7 --- 2023.01.23 ---
// LICENSE
//
...
// 20230123 PHR Corrected missing spaces in alert texts.
...
#		while (strpos($beskrivelse[$x],"''")) $beskrivelse[$x]=str_replace("''","'",$beskrivelse[$x]); #20130731
		...
		if ($submit!='lookup' && (!$bilag[$x] || $bilag[$x]==$bilag[$x-1] || $bilag[$x]-1==$bilag[$x-1]) && (!$dato[$x] || $dato[$x]==$dato[$x-1]) && (!$beskrivelse[$x] || $beskrivelse[$x]==$beskrivelse[$x-1]) && !$d_type[$x] && !$k_type[$x] && !$debet[$x] && !$kredit[$x] && !$faktura[$x] && !$belob[$x]) $bilag[$x]='-';#20170418
...
if (!$simuler) {
	...
	if (!$udskriv) {
		...
		if ($menu=='T') {
			...
			print "<div id=\"header\"> 
			<div class=\"headerbtnLft\"><a title=\"".findtekst(1558, $sprog_id)."\" class=\"button red small left\" href=\"../finans/kladdeliste.php?exitDraft=$kladde_id\" accesskey=\"L\">".findtekst(30, $sprog_id)."</a></div>
			...
			} else print "<a href=\"javascript:confirmClose('../finans/kladdeliste.php?exitDraft=$kladde_id','$tekst')\" accesskey=\"L\">".findtekst(30, $sprog_id)."</a></td>";
...
#		$projekt=$projekt*1;
		...
		if ($id && !$fejl && $valuta!='DKK') {
			...
			if (!$fejl && $row= db_fetch_array(db_select("select kurs from valuta where gruppe='$valutakode' and valdate <= '$valdate' order by valdate",__FILE__ . " linje " . __LINE__))) {
				if ($row['kurs'])
					$qtxt = "update kassekladde set valutakurs = '$row[kurs]' where id = '$id'";
					db_modify($qtxt,__FILE__ . " linje " . __LINE__);
			}
		}
		if ($lobenr) {
